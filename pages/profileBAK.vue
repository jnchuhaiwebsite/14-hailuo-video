  ...
  <div class="min-h-screen bg-blue-pale/80 backdrop-blur-sm pt-[80px]">
    ...
    <div class="max-w-7xl mx-auto bg-gray-800 p-4 sm:p-6 rounded-lg shadow-sm mb-4 sm:mb-6">
      ...
      <!-- <div class="flex flex-col items-center space-y-4">
        ...
        <div class="text-center">
          <div class="flex flex-col items-center space-y-2">
            <h2 class="text-xl sm:text-2xl font-bold text-white">{{ userInfo.nickname }}</h2>
      ...
      <div class="mt-6 sm:mt-8">
        <div class="bg-gradient-to-r from-gray-800 to-gray-700 p-4 sm:p-6 rounded-xl border border-gray-700 shadow-sm">
          <div class="flex flex-col sm:flex-row items-center gap-4">
            <div class="flex items-center gap-4 w-full sm:w-auto">
              <div class="bg-gray-700 p-3 rounded-full">
                <SparklesIcon class="h-6 w-6 sm:h-8 sm:w-8 text-green-400" />
              </div>
              <div class="flex-1 sm:flex-none">
                <p class="text-gray-300 text-xs sm:text-sm font-medium">Remaining Credits</p>
                <p class="text-2xl sm:text-3xl font-bold text-green-400 mt-1">{{ userInfo.free_limit + userInfo.remaining_limit }}</p>
              </div>
            </div>
            <!-- 添加查看记录按钮 -->
            <button 
              @click="openCreditRecords"
              class="w-full sm:w-auto mt-4 sm:mt-0 sm:ml-auto px-4 py-2 bg-gray-700 hover:bg-gray-600 rounded-lg text-gray-200 transition-colors flex items-center justify-center gap-2"
      ...
      <!-- 分享链接 -->
      <div v-if="hasPromotionPermission" class="mt-6 bg-gradient-to-r from-gray-800 to-gray-700 p-4 sm:p-6 rounded-xl border border-gray-700 shadow-sm">
        <div class="flex flex-col sm:flex-row items-start gap-4">
          <div class="flex items-center gap-4 w-full sm:w-auto">
            <div class="bg-gray-700 p-3 rounded-full">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 sm:h-8 sm:w-8 text-purple-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            ...
            <div class="flex-1 sm:flex-none">
              <p class="text-gray-300 text-xs sm:text-sm font-medium">Share & Earn</p>
            </div>
          </div>
          <div class="w-full sm:flex-1">
            <div class="flex flex-col sm:flex-row items-stretch gap-2">
              <div class="flex-1 bg-gray-900 rounded-lg px-3 sm:px-4 py-2 text-gray-300 text-xs sm:text-sm truncate min-h-[40px] flex items-center">
                {{ promotionLink || 'Loading...' }}
              </div>
              <button 
                @click="copyPromotionLink"
                class="w-full sm:w-auto px-4 py-2 bg-purple-600 hover:bg-purple-700 rounded-lg text-white transition-colors flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
      ...
      <div v-if="userInfo.vip_last_time > 0" class="mt-6 bg-gradient-to-r from-gray-800 to-gray-700 p-4 rounded-lg border border-gray-700">
        <div class="flex flex-col sm:flex-row items-start sm:items-center space-y-2 sm:space-y-0 sm:space-x-2">
          <div class="flex items-center space-x-2">
            ...
            <p class="text-yellow-100 font-medium text-sm sm:text-base">VIP Membership Valid Until: {{ formatDate(userInfo.vip_last_time) }}</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-if="showCreditRecords" 
      ...
      style="position: fixed !important; top: 0 !important; left: 0 !important; right: 0 !important; bottom: 0 !important; z-index: 99999 !important; display: flex !important; align-items: center !important; justify-content: center !important; width: 100vw !important; height: 100vh !important;"
    >
      <div class="relative w-full max-w-3xl bg-gray-800/95 backdrop-blur-md rounded-2xl shadow-2xl overflow-hidden max-h-[90vh]" 
        @click.stop
      >
        <!-- 弹框头部 -->
        <div class="flex items-center justify-between p-4 sm:p-6 border-b border-gray-700/50">
          <h3 class="text-lg sm:text-xl font-bold text-white">Credit History</h3>
          <button 
            @click="closeCreditRecords"
        ...
        <!-- 弹框内容 -->
        <div class="p-4 sm:p-6 overflow-y-auto max-h-[70vh] custom-scrollbar" @wheel.stop @touchmove.stop>
          <!-- 积分记录列表 -->
          <div class="space-y-4">
            <div v-for="record in creditRecords" :key="record.id" 
              class="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 bg-gray-700/50 backdrop-blur-sm rounded-xl hover:bg-gray-700 transition-all duration-300 space-y-3 sm:space-y-0">
              <div class="flex items-center space-x-4 w-full sm:w-auto">
                ...
                <div>
                  ...
                  <p class="text-gray-400 text-xs sm:text-sm">{{ formatDate(record.created_at) }}</p>
                </div>
              </div>
          ...
          <!-- 加载状态 -->
          <div v-if="creditLoading" class="text-center py-6 sm:py-8">
            <div class="inline-block animate-spin rounded-full h-6 w-6 sm:h-8 sm:w-8 border-4 border-gray-700 border-t-blue-500"></div>
          </div>

          <!-- 空状态 -->
          <div v-if="!creditLoading && creditRecords.length === 0" class="text-center py-8 sm:py-12">
            <div class="inline-block p-3 sm:p-4 rounded-full bg-gray-700/50 mb-3 sm:mb-4">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 sm:h-8 sm:w-8 text-gray-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                <path d="M12 8v4l3 3m6-3a9 9 0 1 1-18 0 9 9 0 0 1 18 0z"/>
              </svg>
            </div>
            <p class="text-gray-400 text-sm sm:text-base">No credit history yet</p>
          </div>
          ...
          <div v-if="!creditLoading && creditRecords.length > 0" class="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-2">
              <span class="text-gray-400 text-xs sm:text-sm">Show</span>
              <select 
                v-model="creditPageSize"
                @change="handlePageSizeChange(Number(creditPageSize))"
                class="bg-gray-700 text-gray-200 rounded-lg px-2 sm:px-3 py-1.5 text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
              ...
              <span class="text-gray-400 text-xs sm:text-sm">entries</span>
            </div>
            ...
            <div class="flex items-center gap-1 sm:gap-2">
              <button 
                ...
                class="px-2 sm:px-3 py-1.5 rounded-lg text-xs sm:text-sm transition-colors"
                :class="[
                  creditPage === 1 
              ...
              <div class="flex items-center gap-1">
                <button 
                  ...
                  class="w-6 h-6 sm:w-8 sm:h-8 rounded-lg text-xs sm:text-sm transition-colors"
                  :class="[
                    page === creditPage 
    ...
    >
      ...
      >
        <!-- 弹框头部 -->
        <div class="flex items-center justify-between p-4 sm:p-6 border-b border-gray-700/50">
          <h3 class="text-lg sm:text-xl font-bold text-white">Score Records</h3>
          <button 
            @click="closeScoreRecords"
            class="text-gray-400 hover:text-white transition-colors"
          >
        ...
        <!-- 弹框内容 -->
        <div class="p-4 sm:p-6 overflow-y-auto max-h-[70vh] custom-scrollbar">
          <!-- 积分记录列表 -->
          <div class="space-y-4">
            <div v-for="record in scoreRecords" :key="record.id" 
              ...
              <div class="flex items-center space-x-4 w-full sm:w-auto">
                ...
                <div class="flex-1 sm:flex-none">
                  <p class="text-white font-medium text-sm sm:text-base">{{ record.from_user_nickname }}</p>
                  <p class="text-gray-400 text-xs sm:text-sm">{{ formatDate(record.updated_at) }}</p>
                </div>
              </div>
              <!-- 积分信息 -->
              <div class="text-right w-full sm:w-auto">
                ...
                <p class="text-xs sm:text-sm text-gray-400">{{ record.status_text }}</p>
              </div>
            </div>
          </div>

          <!-- 加载状态 -->
          <div v-if="scoreLoading" class="text-center py-6 sm:py-8">
            <div class="inline-block animate-spin rounded-full h-6 w-6 sm:h-8 sm:w-8 border-4 border-gray-700 border-t-blue-500"></div>
          </div>

          <!-- 空状态 -->
          <div v-if="!scoreLoading && scoreRecords.length === 0" class="text-center py-8 sm:py-12">
            <div class="inline-block p-3 sm:p-4 rounded-full bg-gray-700/50 mb-3 sm:mb-4">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 sm:h-8 sm:w-8 text-gray-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                <path d="M12 8v4l3 3m6-3a9 9 0 1 1-18 0 9 9 0 0 1 18 0z"/>
              </svg>
            </div>
            <p class="text-gray-400 text-sm sm:text-base">No score records yet</p>
          </div>
          ...
          <div v-if="!scoreLoading && scoreRecords.length > 0" class="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-2">
              ...
              <select 
                ...
                @change="handleScorePageSizeChange(Number(scorePageSize))"
                class="bg-gray-700 text-gray-200 rounded-lg px-2 sm:px-3 py-1.5 text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
                <option value="10">10</option>
                <option value="20">20</option>
    ...
    <!-- 作品列表 -->
    <div id="works-section" class="max-w-7xl mx-auto px-4 py-6 sm:py-8">
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 space-y-4 sm:space-y-0">
        <h3 class="text-lg sm:text-xl font-bold text-white">My Works</h3>
        <button 
          @click="handleRefresh"
          class="flex items-center gap-2 px-4 py-2 bg-gray-800 rounded-lg shadow-sm hover:shadow-md transition-all text-gray-200 hover:bg-gray-700 w-full sm:w-auto justify-center"
          :disabled="loading"
        >
      ...
      <div class="flex flex-wrap gap-2 sm:gap-4 mb-6">
        <button 
          ...
          class="px-4 sm:px-6 py-2 rounded-full transition-all text-sm sm:text-base"
          :class="[
      ...
      <div v-if="currentTab === 1 || currentTab === 0">
        <!-- 瀑布流布局 -->
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4" v-if="works.length > 0">
              ...
              <template v-if="currentTab === 1">
                <div class="relative w-full h-full">
                  ...
                  <div v-if="videoLoadingStates.get(work.task_id)" class="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center">
                    <div class="text-center">
                      <div class="inline-block animate-spin rounded-full h-6 w-6 sm:h-8 sm:w-8 border-4 border-gray-300 border-t-white mb-2"></div>
                      <p class="text-white text-xs sm:text-sm">Loading...</p>
                    </div>
                  </div>
                </div>
                <div class="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity flex gap-1 sm:gap-2">
                  <button 
                    @click="openLightbox(work.quality_image)"
                    class="bg-gray-800 bg-opacity-80 p-1.5 sm:p-2 rounded-full hover:bg-opacity-100 transition-all shadow-lg hover:shadow-xl"
                    title="View full size"
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 sm:h-5 sm:w-5 text-gray-200" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                  ...
                  >
                    <svg v-if="!isDownloading" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 sm:h-5 sm:w-5 text-gray-200" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                      <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/>
                      <polyline points="7 10 12 15 17 10"/>
                      <line x1="12" y1="15" x2="12" y2="3"/>
                    </svg>
                    <div v-else class="w-4 h-4 sm:w-5 sm:h-5 border-2 border-primary border-t-transparent rounded-full animate-spin"></div>
                  </button>
                </div>
              </template>
              <template v-else>
                <div class="relative w-full h-full">
                  <template v-if="work.origin_image">
                    ...
                    <div class="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center">
                      <div class="text-center">
                        <div class="inline-block animate-spin rounded-full h-6 w-6 sm:h-8 sm:w-8 border-4 border-gray-300 border-t-white mb-2"></div>
                        <p class="text-white text-xs sm:text-sm">Processing...</p>
                      </div>
                    </div>
                  </template>
                  <template v-else>
                    <div class="w-full h-full bg-gray-800 flex items-center justify-center">
                      <div class="text-center">
                        <div class="inline-block animate-spin rounded-full h-6 w-6 sm:h-8 sm:w-8 border-4 border-gray-300 border-t-white mb-2"></div>
                        <p class="text-white text-xs sm:text-sm">Generating...</p>
                      </div>
                    </div>
                  </template>
                </div>
              </template>
              <div class="absolute bottom-0 left-0 right-0 bg-black bg-opacity-70 text-white p-2 sm:p-3">
                <div class="flex justify-between items-start">
                  <p class="text-xs sm:text-sm mt-1">{{ formatDate(work.created_at) }}</p>
                  <button 
                    v-if="work.prompt"
                ...
                <p v-if="work.prompt" class="text-xs sm:text-sm text-gray-300 mt-2 line-clamp-2">{{ work.prompt }}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <!-- 空状态 -->
        <div v-else-if="!loading && works.length === 0" class="text-center py-8 sm:py-12">
          <p class="text-gray-400 text-sm sm:text-base">No works yet</p>
        </div>

        <!-- 加载状态 -->
        <div v-if="loading" class="text-center py-6 sm:py-8 mt-4">
          <div class="inline-block animate-spin rounded-full h-6 w-6 sm:h-8 sm:w-8 border-4 border-gray-700 border-t-blue-500"></div>
        </div>
    ...
    <!-- 分享用户列表 -->
    <div v-if="hasPromotionPermission" class="max-w-7xl mx-auto px-4 py-6 sm:py-8">
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 space-y-4 sm:space-y-0">
        <h3 class="text-lg sm:text-xl font-bold text-white">My invited users</h3>
        <button 
      ...
      <div class="bg-gray-800 rounded-lg p-4 sm:p-6">
        ...
        <div class="space-y-4">
          <div v-if="sharedUsers.length > 0">
            <div v-for="user in sharedUsers" :key="user.user_id" 
              ...
              <div class="flex items-center space-x-4 w-full sm:w-auto">
                ...
                <div class="flex-1 sm:flex-none">
                  <p class="text-white font-medium text-sm sm:text-base">{{ user.user_nickname || 'Anonymous User' }}</p>
                  <p class="text-gray-400 text-xs sm:text-sm">Joined: {{ formatDate(user.user_created_time) }}</p>
                </div>
              </div>
              <!-- 添加总积分显示 -->
              <div class="flex items-center space-x-2 w-full sm:w-auto justify-between sm:justify-end">
                ...
                <button 
                  ...
                  class="px-2 py-1 bg-gray-700 hover:bg-gray-600 rounded text-xs sm:text-sm text-gray-200 transition-colors flex items-center gap-1"
                >
          ...
          <!-- 空状态 -->
          <div v-else-if="!sharedUsersLoading" class="text-center py-8 sm:py-12">
            <div class="inline-block p-3 sm:p-4 rounded-full bg-gray-700/50 mb-3 sm:mb-4">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 sm:h-8 sm:w-8 text-gray-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            ...
            <p class="text-gray-400 text-sm sm:text-base">No shared users yet</p>
          </div>

          <!-- 加载状态 -->
          <div v-if="sharedUsersLoading" class="text-center py-6 sm:py-8">
            <div class="inline-block animate-spin rounded-full h-6 w-6 sm:h-8 sm:w-8 border-4 border-gray-700 border-t-blue-500"></div>
          </div>
    ...
    <div v-if="showPreview" class="fixed inset-0 z-[100] flex items-center justify-center bg-black/95" @click="closePreview">
      <div class="relative w-full h-full flex items-center justify-center p-2 sm:p-4">
        <div class="relative w-full h-full flex items-center justify-center">
          ...
          <button 
            @click="closePreview"
            class="absolute top-2 right-2 sm:top-4 sm:right-4 text-white bg-gray-800 p-2 rounded-full hover:bg-gray-700 transition-colors"
          >
...
const openCreditRecords = () => {
  console.log('Opening credit records modal')
  ...
  // 强制重新渲染
  nextTick(() => {
    console.log('Modal should be visible now, showCreditRecords:', showCreditRecords.value)
    // 检查弹框是否真的显示
    setTimeout(() => {
      ...
      if (modal) {
        ...
        console.log('Modal is visible:', modalElement.style.display, modalElement.style.visibility)
      }
    }, 100)
  })
}
...
const closeCreditRecords = () => {
  console.log('Closing credit records modal')
...
watch(showCreditRecords, (newValue) => {
  console.log('Credit records modal state changed:', newValue)
  if (!newValue) {
    // 当弹框关闭时恢复滚动
    document.body.style.overflow = ''
  }
...
const openScoreRecords = (userId: number) => {
  console.log('Opening score records modal for user:', userId)
...
const closeScoreRecords = () => {
  console.log('Closing score records modal')
...
@media (max-width: 640px) {
  ...
  /* 强制确保弹框显示 */
  [v-if="showCreditRecords"] {
    display: flex !important;
    visibility: visible !important;
    opacity: 1 !important;
  }
  
  [v-if="showScoreRecords"] {
    display: flex !important;
    visibility: visible !important;
    opacity: 1 !important;
  }
  ...
  /* 强制确保弹框在所有情况下都显示 */
  .fixed.inset-0[v-if="showCreditRecords"],
  .fixed.inset-0[v-if="showScoreRecords"] {
    display: flex !important;
    visibility: visible !important;
    opacity: 1 !important;
    position: fixed !important;
